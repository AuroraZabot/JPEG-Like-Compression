function [outComp, ratio] = JPEGcomp(imagePix, compressionFactor)
%-----------
...
if (nargin == 2 && compressionFactor <= 0)
    error("Input quantity must be greater than zero");
end
%-----------
...
%the qualities, I've found that: >90% is considered max quality, from 80 to 
...
%-----------
%In this version of the code we would like to develop a more similar
%implementation to the JPEG approach. We actually use the same approach,
%using runlenght coding after a zigzag scan, then coding through the JPEG 
%Huffman Tables; after this, we just need to decode each block and perform
%the inverse zig-zag scan. The only difference with JPEG is that we don't
%make any diversity coding the AC and the DC part.
%-----------
%Zig-Zag Scan and Huffman encoding on Runlength in the coding part
%-----------
%Inverse Huffman and inverse Zig-Zag Scan in the decoding part
%-----------
...
for i = 1:8:rowsQY
    for j = 1:8:colsQY
        ...
        ZZY = zigzag(block);
        [RY, SY, NZY] = encode(ZZY);
        [encodedY, bitsY] = huffman(RY, SY, NZY);
        bitstreamY = bitstreamY + bitsY;
        
        decodedY(i:i+7, j:j+7) = ihuffman(encodedY);
    end
end
...
for i = 1:8:rowsQCb
    for j = 1:8:colsQCb
        ...
        ZZCb = zigzag(block);
        [RCb, SCb, NZCb] = encode(ZZCb);
        [encodedCb, bitsCb] = huffman(RCb, SCb, NZCb);
        bitstreamCb = bitstreamCb + bitsCb;
        
        decodedCb(i:i+7, j:j+7) = ihuffman(encodedCb);
    end
end
...
for i = 1:8:rowsQCr
    for j = 1:8:colsQCr
        ...
        ZZCr = zigzag(block);
        [RCr, SCr, NZCr] = encode(ZZCr);
        [encodedCr, bitsCr] = huffman(RCr, SCr, NZCr);
        bitstreamCr = bitstreamCr + bitsCr;
        
        decodedCr(i:i+7, j:j+7) = ihuffman(encodedCr);
    end
end

compressedBitstream = bitstreamY + bitstreamCb + bitstreamCr;
%-----------
%After the coding and decoding procedure, we need to dequantize our
